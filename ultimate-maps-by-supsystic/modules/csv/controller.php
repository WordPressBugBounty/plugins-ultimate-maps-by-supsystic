...
class csvControllerUms extends controllerUms
{
  public function exportMaps()
  {
    ...
    $fileDate = str_replace(['/', '.', ':'], '_', date(UMS_DATE_FORMAT_HIS));
    $fileName = sprintf(__('Maps from %s - %s', UMS_LANG_CODE), get_bloginfo('name'), $fileDate);
    $maps = frameUms::_()->getModule('maps')->getModel()->getAllMaps([]); // Only maps data
    if (empty($maps)) {
    ...
    $c = $r = 0;
    $this->_connectCsvLib();
    $csvGenerator = toeCreateObjUms('csvgeneratorUms', [$fileName]);
  ...
  public function exportMarkers()
  {
    $data = reqUms::get('get');
    $delimiter = !empty($data['delimiter']) ? $data['delimiter'] : ';';
    $fileSiteDate = str_replace(['/', '.', ':'], '_', esc_html(get_bloginfo('name')) . ' - ' . date(UMS_DATE_FORMAT_HIS));
    ...
    $c = $r = 0;
    $keys = ['id', 'map_id', 'title', 'description', 'coord_x', 'coord_y'];
    $marker_keys = [];
  ...
  public function import()
  {
    ...
    $res = new responseUms();
    $this->_connectCsvLib();
    $csvGenerator = toeCreateObjUms('csvgeneratorUms', ['']);
                ...
                foreach ($keys as $j => $key) {
                  ...
                  if (strpos($key, '.')) {
                    ...
                    if ($realPreKey == 'map_center') {
                      $valueMapCenter = isset($map['map_center']) ? $map['map_center'] : [];
  ...
  private function _getKeys($data, $prefix = [])
  {
    $keys = [];
    foreach ($data as $k => $v) {
      if (is_array($v)) {
  ...
  {
    return ['exportMaps', 'exportMaps', 'import', 'saveCsvOptions'];
  }
  public function getPermissions()
  {
    return [
      UMS_USERLEVELS => [
        UMS_ADMIN => ['exportMaps', 'exportMaps', 'import', 'saveCsvOptions'],
      ],
    ];
  }
}
