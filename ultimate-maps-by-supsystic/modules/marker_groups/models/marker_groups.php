...
class marker_groupsModelUms extends modelUms
{
  ...
  public function getAllMarkerGroups($d = [])
  {
  ...
  public function remove($markerGroupId)
  {
    ...
    if (!empty($markerGroupId)) {
      ...
      if ($deleteMarkerGroup) {
        ...
        $data_update = ['marker_group_id' => 0];
        $data_where = ['marker_group_id' => $markerGroupId];
  ...
  protected function _dataSave($data, $update = false)
  {
    ...
    $mgrParamsKeys = ['bg_color', 'text_color', 'claster_icon', 'claster_icon_width', 'claster_icon_height'];
    $mgrParams = [];
    ...
    if (!empty($params)) {
      ...
      if ($this->_validateSaveMarkerGroup($insertData)) {
        ...
        if (!empty($newMarkerGroupId) && $newMarkerGroupId) {
          ...
          array_splice($allMarkerGroupsIds, $offset + 1, 0, [(string) $newMarkerGroupId]);
  ...
  public function getChildrenList($groups, $parent, $children = [])
  {
    foreach ($groups as $g) {
      if ($g['parent'] == $parent) {
  ...
  public function resortMarkerGroups($markerGroupsIds = [])
  {
    if ($markerGroupsIds) {
      $i = 1;
      global $wpdb;
  ...
  {
    $uncategorized = $withUncategorized ? $this->getUncategorizedGroupData() : [];
    $markerGroupsTree = [];
  ...
  public function getMarkerGroupsTree($groups, $parentId = 0)
  {
    ...
    foreach ($groups as $i => $g) {
      if (isset($g['parent']) && $g['parent'] == $parentId) {
        $res[] = array_merge($g, ['children' => $this->getMarkerGroupsTree($groups, $g['id'])]);
      }
    }
    return $res;
  }
  ...
  public function getMarkerGroupsForSelect($markerGroupsForSelect = [], $current_group_id = 0)
  {
