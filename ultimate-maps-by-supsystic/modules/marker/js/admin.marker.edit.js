...
jQuery(window).on('load',function(){
	...
	,	loadComplete: function(res) {
		...
		update: function (e, ui) {
			...
			jQuery.sendFormUms({
				data: { mod: 'maps',  '_wpnonce': UMS_NONCE['ums_nonce'], action: 'resortMarkers', markers_list: markersList }
	...
	jQuery('#umsMarkerDeleteBtn').click(function(){
		...
		if(confirm('Remove '+ markerTitle+ ' marker?')) {
			...
			if(currentMarkerIdInForm) {
				jQuery.sendFormUms({
					btn: this
					,	data: {action: 'removeMarker', '_wpnonce': UMS_NONCE['ums_nonce'], mod: 'marker', id: currentMarkerIdInForm}
					,	onSuccess: function(res) {
						if(!res.error) {
							removeFinalClb();
						}
...
function umsRemoveMarkerFromMapTblClick(markerId, params) {
	...
	jQuery.sendFormUms({
		btn: btn
		,	data: {action: 'removeMarker',  '_wpnonce': UMS_NONCE['ums_nonce'], mod: 'marker', id: markerId}
		,	onSuccess: function(res) {
			if(!res.error){
...
function _umsMarkerDragEndClb() {
	...
	if(draggedId) {	// Just save it in database
		jQuery.sendFormUms({
			data: {mod: 'marker', action: 'updatePos',  '_wpnonce': UMS_NONCE['ums_nonce'], id: draggedId, lat: this.lat(), lng: this.lng()}
		,	onSuccess: function(res) {
				if(!res.error) {
					jQuery('#umsMarkersListGrid').trigger('reloadGrid');
				}
...
function umsInitIconsWnd() {
	...
	jQuery('#umsIconsWnd').on('click', '.previewIcon i', function(e){
      ...
      if(confirm('Remove icon with ID: '+ iconId+ '? If you delete this icon, all markers that use it will no longer be able to use this image.')) {
   		jQuery.sendFormUms({
   			data: {action: 'remove',  '_wpnonce': UMS_NONCE['ums_nonce'], mod: 'icons', id: iconId}
   		,	onSuccess: function(res) {
   				iconWrapper.remove();
   			}
   		});
