  ...
  g_umsMapLoadObserver.trigger(umsGetMapsEngine(umsMainMap), function () {
    g_umsGrid
      .jqGrid({
        ...
        loadComplete: function (res) {
          if (g_umsGridData === null) g_umsGridData = res.rows;
    ...
    jQuery('#umsAddNewMarkerBtn').click(function () {
      ...
      if (!currentEditId) {
        ...
        if (title && title != '') {
          // Save it if there was some required changes
    ...
    jQuery('#umsMarkerForm')
      .find('input[name="marker_opts[coord_x]"],input[name="marker_opts[coord_y]"]')
      .change(function () {
        ...
        if (marker) {
          ...
          // If there are no marker on map - set it and re-position it right into new position
...
function umsResetMarkerForm() {
  ...
  if (jQuery('#umsMarkerForm input[name="marker_opts[params][description_mouse_hover]"]').prop('checked') === false) umsHideCloseDescriptionCheckbox();
  ...
  return jQuery('#umsIconsWnd').dialog({
    ...
    beforeClose: function (e, ui) {
      for (var i in dialodClasses) {
        if (jQuery(this).hasClass(dialodClasses[i])) jQuery(this).removeClass(dialodClasses[i]);
      }
    },
  });
}
