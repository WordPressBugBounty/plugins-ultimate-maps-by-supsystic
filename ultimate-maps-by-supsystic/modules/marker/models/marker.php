...
class markerModelUms extends modelUms
{
  ...
  public function _afterGet($marker, $widthMapData = false, $withoutIcons = false)
  {
    if (!empty($marker)) {
      ...
      if (!empty($marker['params']['title_is_link'])) {
  ...
  public function getAllMarkers($d = [], $widthMapData = false)
  {
    global $wpdb;
  ...
  public function getListForTblBySearch($search, $limitStart, $rowsLimit, $mapId)
  {
    global $wpdb;
    if (!empty($search)) {
      $data = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}ums_markers WHERE " . $wpdb->prepare(' map_id = %s AND (id = %s OR title = %s) ORDER BY id ASC LIMIT %1s,%1s', $mapId, $search, $search, (int) $limitStart, (int) $rowsLimit), ARRAY_A);
    } else {
