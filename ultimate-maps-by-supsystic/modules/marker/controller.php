...
class markerControllerUms extends controllerUms
{
  ...
  {
    if (!empty($data)) {
      $markersIds = ['map_id' => $data[0]['map_id'], 'markers_list' => []];
      foreach ($data as $i => $m) {
        ...
        // Marker Coordinates
        $coords =
          '<div class="egm-marker-latlng">' .
          round($data[$i]['coord_x'], 2) .
          '"N ' .
          round($data[$i]['coord_y'], 2) .
          '"E
					</div>';
  ...
  public function getPermissions()
  {
    return [
      UMS_USERLEVELS => [
        UMS_ADMIN => ['save', 'removeMarker', 'getMarkerForm', 'getListForTable', 'getMarker', 'removeList', 'getMapMarkers', 'updatePos'],
      ],
    ];
  }
}
