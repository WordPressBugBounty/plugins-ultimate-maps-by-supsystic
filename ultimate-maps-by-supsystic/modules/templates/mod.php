...
class templatesUms extends moduleUms
{
  ...
  {
    frameUms::_()->addScript('jquery-ui-dialog', '', ['jquery']);
    frameUms::_()->addScript('jquery-ui-slider', '', ['jquery']);
    frameUms::_()->addScript('wp-color-picker');
    frameUms::_()->addScript('icheck', UMS_JS_PATH . 'icheck.min.js');
    frameUms::_()->addScript('jquery-ui-autocomplete', '', ['jquery']);
  ...
  public function loadCoreJs()
  {
    ...
    if (!$loaded) {
      ...
      frameUms::_()->addScript('commonUms', UMS_JS_PATH . 'common.js', ['jquery']);
      frameUms::_()->addScript('coreUms', UMS_JS_PATH . 'core.js', ['jquery']);
  ...
  {
    $this->_styles = [
      'styleUms' => ['path' => UMS_CSS_PATH . 'style.css', 'for' => 'admin'],
      'supsystic-uiUms' => ['path' => UMS_CSS_PATH . 'supsystic-ui.css', 'for' => 'admin'],
      'dashicons' => ['for' => 'admin'],

      'icheck' => ['path' => UMS_CSS_PATH . 'jquery.icheck.css', 'for' => 'admin'],
  ...
  public function loadGoogleFont($font)
  {
    ...
    if (!isset($loaded[$font])) {
      frameUms::_()->addStyle('google.font.' . str_replace([' '], '-', $font), 'https://fonts.googleapis.com/css?family=' . urlencode($font));
      $loaded[$font] = 1;
    }
  }
}
