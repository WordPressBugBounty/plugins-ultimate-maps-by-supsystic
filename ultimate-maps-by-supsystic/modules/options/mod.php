...
class optionsUms extends moduleUms
{
  ...
  {
    dispatcherUms::addAction('afterModulesInit', [$this, 'initAllOptValues']);
    dispatcherUms::addFilter('mainAdminTabs', [$this, 'addAdminTab']);
    dispatcherUms::addAction('adminMenuAccessCap', [$this, 'adminMenuAccessRoles'], 10, 1);
  }
  public function addAdminTab($tabs)
  {
    $tabs['settings'] = [
      'label' => __('Settings', UMS_LANG_CODE),
  ...
  {
    if (empty($this->_tabs)) {
      $this->_tabs = dispatcherUms::applyFilters('mainAdminTabs', [
  ...
  public function getAll()
  {
    if (empty($this->_options)) {
      $this->_options = dispatcherUms::applyFilters('optionsDefine', [
        'general' => [
          'label' => __('General', UMS_LANG_CODE),
          'opts' => [
            'def_engine' => [
              'label' => __('Maps Engine', UMS_LANG_CODE),
              'desc' => __('Select required Maps Engine to drive your maps.', UMS_LANG_CODE),
              'def' => 'leaflet',
              'html' => 'selectbox',
            ...
            'bing_key' => [
              'label' => __('Bing Maps API Key', UMS_LANG_CODE),
              'desc' => sprintf(__('Get your Bing Maps API Key under your Microsoft Bing account - check it <a href="%s" target="_blank">here</a>', UMS_LANG_CODE), 'https://www.bingmapsportal.com/Application'),
              'def' => '',
              'html' => 'text' /*, 'connect' => 'def_engine:bing'*/,
            ],
            'rapidapi_key' => [
              'label' => __('MapTiles RapidAPI Key', UMS_LANG_CODE),
              'desc' => sprintf(__('Get your Rapid API key free or paided <a href="%s" target="_blank">here</a>', UMS_LANG_CODE), 'https://rapidapi.com/MapTilesApi/api/maptiles/pricing/'),
              'def' => '',
              'html' => 'text' /*, 'connect' => 'def_engine:bing'*/,
            ],
            'mapbox_key' => [
              'label' => __('MapBox Access Token', UMS_LANG_CODE),
              'desc' => sprintf(__('Get your MapBox Access Token under your MapBox account - check it <a href="%s" target="_blank">here</a>', UMS_LANG_CODE), 'https://www.mapbox.com/account/'),
              'def' => '',
              'html' => 'text' /*, 'connect' => 'def_engine:mapbox'*/,
            ],
            'thunderforest_key' => [
              'label' => __('Thunderforest API Key', UMS_LANG_CODE),
              'desc' => sprintf(__('Get your Thunderforest API Key under your Thunderforest account - check it <a href="%s" target="_blank">here</a>', UMS_LANG_CODE), 'https://manage.thunderforest.com/dashboard'),
              'def' => '',
              'html' => 'text' /*, 'connect' => 'def_engine:mapbox'*/,
            ],
            ...
            'add_love_link' => [
              'label' => __('Show promo link', UMS_LANG_CODE),
              'desc' => __('To promote our plugin, you can show the promo link below the table, if you don\'t want to show the promo link you can also hide it. You can completely remove the link under the table by purchasing the PRO version of the plugin. Thank you for cooperation!', UMS_LANG_CODE),
              'def' => '0',
              'html' => 'checkboxHiddenVal',
            ],
            'remove_love_link' => [
              'label' => __('Remove promo link', UMS_LANG_CODE),
              'desc' => __('If you want to delete promo link below the table from HTML just check this box. Only for PRO users.', UMS_LANG_CODE),
              'def' => '0',
              'html' => 'checkboxHiddenVal',
              'pro' => '',
            ],
      ...
      foreach ($this->_options as $catKey => $cData) {
        foreach ($cData['opts'] as $optKey => $opt) {
          ...
          if (isset($opt['pro']) && !$isPro) {
            $this->_options[$catKey]['opts'][$optKey]['pro'] = frameUms::_()
              ->getModule('supsystic_promo')
              ->generateMainLink('utm_source=plugin&utm_medium=' . $optKey . '&utm_campaign=ultimatemaps');
          }
        }
      }
