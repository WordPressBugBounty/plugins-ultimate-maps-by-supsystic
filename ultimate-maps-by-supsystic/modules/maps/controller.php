...
class mapsControllerUms extends controllerUms
{
  ...
  public function save()
  {
    ...
    if ($saveRes) {
      ...
      if ($membershipModule) {
        ...
        if ($membershipModel) {
          $membershipModel->updateRow(['maps_id' => $mapId, 'allow_use' => isset($data['map_opts']['membershipEnable']) ? $data['map_opts']['membershipEnable'] : 0]);
        }
      }
  ...
  public function changeEngine()
  {
    ...
    if (!$id) {
      // This is new map - just change it
      $res->addData(
        'new_engine_url',
        frameUms::_()
          ->getModule('options')
  ...
  {
    return ['getListForTbl', 'getAllMaps', 'save', 'clear', 'remove', 'removeGroup', 'cloneMapGroup', 'resortMarkers', 'changeEngine'];
  }
  public function getPermissions()
  {
    return [
      UMS_USERLEVELS => [
        UMS_ADMIN => ['getListForTbl', 'getAllMaps', 'save', 'clear', 'remove', 'removeGroup', 'cloneMapGroup', 'resortMarkers', 'changeEngine'],
      ],
    ];
  }
}
