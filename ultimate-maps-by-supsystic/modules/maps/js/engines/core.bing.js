...
umsBingMap.prototype._beforeInit = function () {
  ...
  if (typeof this._mapParams.navigation_bar_mode !== 'undefined' && Microsoft.Maps.NavigationBarMode[this._mapParams.navigation_bar_mode]) {
    this._mapParams.navigationBarMode = Microsoft.Maps.NavigationBarMode[this._mapParams.navigation_bar_mode];
  }
  if (typeof this._mapParams.map_type !== 'undefined' && Microsoft.Maps.MapTypeId[this._mapParams.map_type]) {
    ...
    params = jQuery.extend(
      {
        ...
        disableScrollWheelZoom: typeof this._mapParams.mouse_wheel_zoom !== 'undefined' && !parseInt(this._mapParams.mouse_wheel_zoom) ? true : false,
      },
      this._mapParams
    );
...
umsBingMap.prototype._afterInit = function () {
  if (typeof this._mapParams.dbl_click_zoom !== 'undefined' && !parseInt(this._mapParams.dbl_click_zoom)) {
    ...
    var clasterGridSize = parseInt(this.getParam('marker_clasterer_grid_size')),
      ...
      opts = {
        clusteredPinCallback: function (clusterPin) {
          ...
          clusterPin.setOptions({
            color: self.getParam('marker_clasterer_background_color'),
          });
        },
      };
    if (clasterGridSize) {
...
umsBingMap.prototype._generateMarkerInfoWndStyle = function () {
  ...
  // Title style
  res.push(this._elementId + ' .infobox-title {color: ' + this.getParam('marker_title_color') + '; font-size: ' + this.getParam('marker_title_size') + 'px}');
  // Description
  res.push(this._elementId + ' .infobox-info, ' + this._elementId + ' .infobox-info * {font-size: ' + this.getParam('marker_desc_size') + 'px}');
  // Background tip
  res.push(this._elementId + ' .MicrosoftMap .Infobox .infobox-stalk {background-image: none;' + 'width: 0;' + 'height: 0;' + 'border-style: solid;' + 'border-width: 16px 7px 0 7px;' + 'border-color: ' + this.getParam('marker_infownd_bg_color') + ' transparent transparent transparent;}');
  // Background
