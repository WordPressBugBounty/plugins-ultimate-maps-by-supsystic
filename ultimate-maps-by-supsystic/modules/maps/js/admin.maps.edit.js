  ...
  g_umsMapLoadObserver.trigger(umsGetMapsEngine(previewMapParams), function () {
    ...
    jQuery('#umsMapForm').submit(function () {
      ...
      $currMapForm.sendFormUms({
        btn: '#umsMapSaveBtn',
        appendData: {
          add_marker_ids: g_umsMapMarkersIdsAdded,
          add_shape_ids: g_umsMapShapesIdsAdded,
        },
    ...
    jQuery('#umsMarkerTab input[name="marker_opts[params][description_mouse_hover]"]').change(function () {
      if (jQuery(this).prop('checked') === true) umsShowCloseDescriptionCheckbox();
      else umsHideCloseDescriptionCheckbox();
    });
  });
...
function umsUpdateClusterIcon(url, width, height) {
  ...
  g_umsMap.setParam('marker_clasterer_icon', url).setParam('marker_clasterer_icon_width', width).setParam('marker_clasterer_icon_height', height).enableClasterization(g_umsMap.getParam('marker_clasterer'));
}
function umsInitMapMarkersListWnd() {
  ...
  jQuery('#umsMarkersListWnd')
    .find('.umsMmlElement')
    .css('max-width', popupWidth - 20);
...
function _umsResizeRightSidebar(container) {
  ...
  if (wndWd > 991) {
    newHeight = wndHt - jQuery('#wpadminbar').outerHeight() - jQuery('#umsMapPreview').outerHeight() - jQuery('.umsControlBtns:first').outerHeight() - jQuery('.ui-jqgrid-htable:first').outerHeight();
  }
...
function umsUnshiftButtons(btns) {
  for (var i in btns) {
    if (jQuery('#' + i).hasClass(btns[i])) jQuery('#' + i).trigger('click');
  }
}
