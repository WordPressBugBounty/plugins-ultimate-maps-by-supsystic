jQuery(document).ready(function(){
	...
	jQuery('#umsMapsRemoveGroupBtn').click(function(){
		...
		if(confirm(confirmMsg)) {
			jQuery.sendFormUms({
				btn: this
			,	data: {mod: 'maps',  '_wpnonce': UMS_NONCE['ums_nonce'], action: 'removeGroup', listIds: listIds}
	...
	jQuery('#umsMapsCloneGroupBtn').click(function(){
		...
		if(confirm(confirmMsg)) {
			jQuery.sendFormUms({
				btn: this
			,	data: {mod: 'maps',  '_wpnonce': UMS_NONCE['ums_nonce'], action: 'cloneMapGroup', listIds: listIds}
			,	onSuccess: function(res) {
					if(!res.error) {
						jQuery('#umsMapsTbl').trigger( 'reloadGrid' );
					}
...
function umsRemoveMapFromTblClick(mapId){
	...
	jQuery.sendFormUms({
		msgElID: msgEl
	,	data: {action: 'remove', mod: 'maps', '_wpnonce': UMS_NONCE['ums_nonce'], id: mapId}
	,	onSuccess: function(res) {
			if(!res.error){
				jQuery('#umsMapsTbl').trigger( 'reloadGrid' );
				setTimeout(function(){
