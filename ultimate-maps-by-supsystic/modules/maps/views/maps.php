...
class mapsViewUms extends viewUms
{
  ...
  private $_mapsObj = [];
  private $_shortCodeHtmlParams = ['width', 'height', 'align'];
  private $_paramsCanNotBeEmpty = ['width', 'height'];
  private $_mapStyles = [];
  private $_displayColumns = [];

  private $_mapsApiUrls = [];
  ...
  public function getApiKey($engine)
  {
    ...
    if (
      $userApiKey = dispatcherUms::applyFilters(
        'rewriteApiKey',
        frameUms::_()
          ->getModule('options')
          ->get($engine . '_key'),
  ...
  private function _getApiUrl($engine)
  {
    if (!isset($this->_mapsApiUrls[$engine])) {
      ...
      $urlParams = dispatcherUms::applyFilters('apiUrlParams', ['key' => $this->getApiKey($engine)]);
  ...
  public function drawMap($params)
  {
    ...
    if (empty($mapObj)) {
      return isset($params['id']) ? sprintf(__('Map with ID %d not found', UMS_LANG_CODE), $params['id']) : __('Map not found', UMS_LANG_CODE);
    }
    ...
    $mapObj['params']['markers_list_type'] = isset($params['markers_list_type']) ? $params['markers_list_type'] : (isset($mapObj['params']['markers_list_type']) && !empty($mapObj['params']['markers_list_type']) ? $mapObj['params']['markers_list_type'] : '');
  ...
  public function applyShortcodeHtmlParams($mapObj, $params)
  {
    ...
    foreach ($this->_shortCodeHtmlParams as $code) {
      if (isset($params[$code])) {
        if (in_array($code, $this->_paramsCanNotBeEmpty) && empty($params[$code])) {
          continue;
        }
  ...
  public function getDisplayColumns()
  {
    if (empty($this->_displayColumns)) {
      $this->_displayColumns = [
        'id' => ['label' => __('ID'), 'db' => 'id'],
        'title' => ['label' => __('Title'), 'db' => 'title'],
        'list_html_options' => ['label' => __('Html options'), 'db' => 'html_options'],
        'list_markers' => ['label' => __('Markers'), 'db' => 'markers'],
        'operations' => ['label' => __('Operations'), 'db' => 'operations'],
      ];
    }
  ...
  public function getTabContent()
  {
    ...
    frameUms::_()->addJSVar('admin.maps.list', 'umsTblDataUrl', uriUms::mod('maps', 'getListForTbl', ['reqType' => 'ajax', '_wpnonce' => wp_create_nonce('ums_nonce')]));
  ...
  public function getEditMap($id = 0)
  {
    ...
    $stylizationsForSelect = ['none' => __('None', UMS_LANG_CODE)];
    ...
    // jqGrid tables urls
    $umsMarkersTblDataUrl = uriUms::mod('marker', 'getListForTbl', ['reqType' => 'ajax', 'map_id' => $id, '_wpnonce' => wp_create_nonce('ums_nonce')]);
    ...
    if ($editMap) {
      ...
      if ($map['markers'] && !empty($map['markers'])) {
        foreach ($map['markers'] as $marker) {
          if ($marker['marker_group_id']) {
            if (in_array($marker['marker_group_id'], $mapMarkersGroupsList)) {
              continue;
            }
      ...
      if ($mapMarkersGroupsList) {
        ...
        foreach ($allMarkerGroupsList as $group) {
          if (in_array($group['id'], $mapMarkersGroupsList)) {
            array_push($mapMarkersGroups, $group);
          }
        }
    ...
    $this->assign(
      'markerGroupsForSelect',
      frameUms::_()
        ->getModule('marker_groups')
        ->getModel()
        ->getMarkerGroupsForSelect(['0' => __('None', UMS_LANG_CODE)]),
    );
    ...
    if ($isPro) {
      // It's not available for old PRO
      $isCustSearchAndMarkersPeriodAvailable = false;
      if (frameUms::_()->getModule('custom_controls') && method_exists(frameUms::_()->getModule('custom_controls'), 'isCustSearchAndMarkersPeriodAvailable') && frameUms::_()->getModule('custom_controls')->isCustSearchAndMarkersPeriodAvailable()) {
        $isCustSearchAndMarkersPeriodAvailable = true;
      }
    }
    ...
    $this->assign('tabs', [
      ...
      'umsMarkerTab' => [
        ...
        'icon' => 'fa-map-marker',
        'btns' => [['id' => 'umsAddNewMarkerBtn', 'label' => __('New', UMS_LANG_CODE)]],
      ...
      'umsShapeTab' => [
        ...
        'icon' => 'fa-cubes',
        'btns' => [['id' => 'umsAddNewShapeBtn', 'classes' => 'umsProOpt', 'label' => __('New', UMS_LANG_CODE)]],
        'content' =>
          $isPro && frameUms::_()->getModule('shape')
            ? frameUms::_()
              ->getModule('shape')
              ->getView()
              ->getEditShapes([
  ...
  public function connectMapsAssets($map, $forAdminArea = false, $params = [])
  {
    ...
    if ((!empty($map) && (isset($map['params']['marker_clasterer']) && $map['params']['marker_clasterer'] != 'none')) || $forAdminArea || ($params && isset($params['add_clasterer']))) {
  ...
  private function _getMapLeaFletTypes($engine)
  {
    ...
    switch ($engine) {
      case 'l-mapbox':
        $types = [
          'streets-v11' => ['label' => __('Streets', UMS_LANG_CODE), 'attr' => '© <a href="https://www.mapbox.com/">Mapbox</a> | © <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>'],
      ...
      case 'l-thunderforest':
        $types = [
          'cycle' => ['label' => __('OpenCycleMap', UMS_LANG_CODE), 'attr' => '© <a href="http://www.thunderforest.com/">Thunderforest</a> | © <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'],
      ...
      case 'leaflet':
        ...
        $types = [
            ...
            'https://tile.openstreetmap.de/{z}/{x}/{y}.png' => [
                'label' => __('OSM Germany', UMS_LANG_CODE),
                'attr' => $osmAttr,
            ],
            'https://{s}.tile.openstreetmap.fr/hot/{z}/{x}/{y}.png' => [
                'label' => __('OSM Humanitarian', UMS_LANG_CODE),
                'attr' => $osmAttr,
            ],
            'https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png' => [
                'label' => __('OpenTopoMap', UMS_LANG_CODE),
                'attr' => 'Map data: ' . $osmAttr . ', <a href="http://viewfinderpanoramas.org">SRTM</a> | Map style: &copy; <a href="https://opentopomap.org">OpenTopoMap</a> (CC-BY-SA)',
            ],
            'https://tileserver.memomaps.de/tilegen/{z}/{x}/{y}.png' => [
                'label' => __('OPNVKarte (Transit)', UMS_LANG_CODE),
                'attr' => 'Map <a href="https://memomaps.de/">memomaps.de</a> CC-BY-SA, map data ' . $osmAttr,
            ],
            ...
            'https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png' => [
                'label' => __('CartoDB Dark Matter', UMS_LANG_CODE),
                'attr' => $cartoAttr,
            ],
            'https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png' => [
                'label' => __('CartoDB Voyager', UMS_LANG_CODE),
                'attr' => $cartoAttr,
            ],
            'https://{s}.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}{r}.png' => [
                'label' => __('CartoDB Positron No Labels', UMS_LANG_CODE),
                'attr' => $cartoAttr,
            ],
            'https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}{r}.png' => [
                'label' => __('CartoDB Dark No Labels', UMS_LANG_CODE),
                'attr' => $cartoAttr,
            ],
            'https://server.arcgisonline.com/ArcGIS/rest/services/World_Street_Map/MapServer/tile/{z}/{y}/{x}' => [
                'label' => __('Esri World Street', UMS_LANG_CODE),
                'attr' => $esriAttr . ' &mdash; Source: Esri, DeLorme, NAVTEQ, USGS, Intermap, iPC, NRCAN, Esri Japan, METI, Esri China (Hong Kong), Esri (Thailand), TomTom, 2012',
            ],
            'https://server.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/{z}/{y}/{x}' => [
                'label' => __('Esri World Topo', UMS_LANG_CODE),
                'attr' => $esriAttr . ' &mdash; Esri, DeLorme, NAVTEQ, TomTom, Intermap, iPC, USGS, FAO, NPS, NRCAN, GeoBase, Kadaster NL, Ordnance Survey, Esri Japan, METI, Esri China (Hong Kong)',
            ],
            'https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}' => [
                'label' => __('Esri Satellite', UMS_LANG_CODE),
                'attr' => $esriAttr . ' &mdash; Source: Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community',
            ],
            'https://server.arcgisonline.com/ArcGIS/rest/services/NatGeo_World_Map/MapServer/tile/{z}/{y}/{x}' => [
                'label' => __('Esri NatGeo', UMS_LANG_CODE),
                'attr' => $esriAttr . ' &mdash; National Geographic, Esri, DeLorme, NAVTEQ, UNEP-WCMC, USGS, NASA, ESA, METI, NRCAN, GEBCO, NOAA, iPC',
            ],
            'https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}' => [
                'label' => __('Esri Gray', UMS_LANG_CODE),
                'attr' => $esriAttr . ' &mdash; Esri, DeLorme, NAVTEQ',
            ],
        ];
        break;
      case 'l-maptiles':
        $types = [
          'en' => ['label' => __('English world map', UMS_LANG_CODE), 'attr' => '© <a href="https://www.maptilesapi.com/">Map Tiles API</a> | © <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>'],
          'es' => ['label' => __('Spanish world map', UMS_LANG_CODE), 'attr' => '© <a href="https://www.maptilesapi.com/">Map Tiles API</a> | © <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>'],
          'fr' => ['label' => __('French world map', UMS_LANG_CODE), 'attr' => '© <a href="https://www.maptilesapi.com/">Map Tiles API</a> | © <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>'],
        ];
        break;
    }
    return $types;
