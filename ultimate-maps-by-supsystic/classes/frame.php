...
class frameUms
{
  ...
  {
    $this->_res = toeCreateObjUms('response', []);
  }
  ...
  {
    reqUms::init();
    // add_filter('script_loader_tag', [$this, 'fixRocketLoader'], 10, 3);
    add_action('init', [$this, '_delayedInit'], 5);
  }
  // public function fixRocketLoader($tag, $handle, $src)
  // {
  //   $syncHandles = ['jquery', 'jquery-core', 'jquery-migrate'];
  //   if (strpos($handle, 'ums') !== false || in_array($handle, $syncHandles)) {
  ...
  public function _delayedInit()
  {
    ...
    register_activation_hook(UMS_DIR . DS . UMS_MAIN_FILE, ['utilsUms', 'activatePlugin']); //See classes/install.php file
    register_uninstall_hook(UMS_DIR . DS . UMS_MAIN_FILE, ['utilsUms', 'deletePlugin']);
    register_deactivation_hook(UMS_DIR . DS . UMS_MAIN_FILE, ['utilsUms', 'deactivatePlugin']);
  ...
  public function havePermissions($code, $action)
  {
    ...
    if ($mod) {
      ...
      if (!empty($permissions)) {
        // Special permissions
        if (isset($permissions[UMS_METHODS]) && !empty($permissions[UMS_METHODS])) {
          foreach ($permissions[UMS_METHODS] as $method => $permissions) {
            // Make case-insensitive
            $permissions[UMS_METHODS][strtolower($method)] = $permissions;
          }
          if (array_key_exists($action, $permissions[UMS_METHODS])) {
            // Permission for this method exists
            $currentUserPosition = frameUms::_()->getModule('user')->getCurrentUserPosition();
            if ((is_array($permissions[UMS_METHODS][$action]) && !in_array($currentUserPosition, $permissions[UMS_METHODS][$action])) || (!is_array($permissions[UMS_METHODS][$action]) && $permissions[UMS_METHODS][$action] != $currentUserPosition)) {
              $res = false;
            }
          }
        }
        if (isset($permissions[UMS_USERLEVELS]) && !empty($permissions[UMS_USERLEVELS])) {
          $currentUserPosition = frameUms::_()->getModule('user')->getCurrentUserPosition();
  ...
  public function addScript($handle, $src = '', $deps = [], $ver = false, $in_footer = false, $vars = [])
  {
    ...
    // add_filter('script_loader_tag', function ($tag, $filterHandle, $filterSrc) use ($handle) {
    //   if ($filterHandle === $handle && strpos($tag, 'data-cfasync="false"') === false) {
    //     return str_replace('<script ', '<script data-cfasync="false" ', $tag);
    //   }
    //   return $tag;
    // }, 10, 3);
    // add_filter('script_loader_tag', function ($tag, $filterHandle, $filterSrc) use ($handle) {
    //   $targets = [$handle, $handle . '-js-extra', $handle . '-extra'];
    //   if (in_array($filterHandle, $targets) && strpos($tag, 'data-cfasync="false"') === false) {
    //     return str_replace('<script ', '<script data-cfasync="false" ', $tag);
    //   }
    //   return $tag;
    // }, 10, 3);
    // add_filter('wp_inline_script_attributes', function ($attributes, $javascript) use ($handle) {
    //   if (isset($attributes['id']) && strpos($attributes['id'], $handle) !== false) {
