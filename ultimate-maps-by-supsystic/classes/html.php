...
class htmlUms
{
  ...
  public static function ajaxfile($name, $params = ['url' => '', 'value' => '', 'fid' => '', 'buttonName' => ''])
  {
    ...
    if (strpos($params['url'], 'pl=' . UMS_CODE) === false) {
      $params['url'] = uriUms::_(['baseUrl' => $params['url'], 'pl' => UMS_CODE]);
    }
  ...
  public static function slider($name, $params = ['value' => 0, 'min' => 0, 'max' => 0, 'step' => 1, 'slide' => '', 'units' => 'meter', 'units_plur' => 'meters'])
  {
    ...
    if (!empty($params)) {
      ...
      foreach ($params as $k => $v) {
        ...
        $value = is_numeric($v) || in_array($k, ['slide', 'create']) ? $v : '"' . $v . '"';
        $paramsArr[] = $k . ': ' . $value;
      }
  ...
  public static function slideInput($name, $params = ['attrs' => '', 'checked' => false, 'id' => ''])
  {
    ...
    return '<a class="toeSlideShellUms" href="#"' .
      $params['attrs'] .
      '>
			<span class="toeSlideButtUms"></span>
			<span class="toeSlideOnUms">' .
      __('ON') .
      '</span>
			<span class="toeSlideOffUms">' .
      __('OFF') .
      '</span>
  ...
  {
    return self::hidden('_wpnonce', ['value' => wp_create_nonce('ums_nonce')]);
  }
  public static function defaultNonceForFront()
  {
    return self::hidden('_wpnonce', ['value' => wp_create_nonce('ums_nonce_frontend')]);
  }
