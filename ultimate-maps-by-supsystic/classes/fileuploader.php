...
class fileuploaderUms
{
  ...
  public function upload($params = [])
  {
    ...
    if ($setRandFileName) {
      add_filter('wp_handle_upload_prefilter', [$this, 'changeFileName']);
    }
    ...
    if (isset($upload['type']) && !empty($upload['type'])) {
      ...
      $this->_fileInfo['url'] = str_replace('[AFTER_PROTOCOL]', '://', str_replace('//', '/', str_replace('://', '[AFTER_PROTOCOL]', str_replace(DS, '/', $upload['url']))));
      $res = true;
    ...
    if ($setRandFileName) {
      remove_filter('wp_handle_upload_prefilter', [$this, 'changeFileName']);
    }
    return $res;
  }
