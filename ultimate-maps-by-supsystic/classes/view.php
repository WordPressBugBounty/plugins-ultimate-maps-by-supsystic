...
abstract class viewUms extends baseObjectUms {
    ...
    public function addUmsNonce() {
        if (!$this->_nonceIsInited) {
            $this->_nonceIsInited = true;
            if (is_admin()) {
                $ums_nonce = wp_create_nonce('ums_nonce');
                $jsData = array(
                'ums_nonce' => $ums_nonce,
                );
                frameUms::_()->addScript('nonceUms', UMS_JS_PATH . 'nonce.js');
                frameUms::_()->addJSVar('nonceUms', 'UMS_NONCE', $jsData);
            } else {
                $ums_nonce = wp_create_nonce('ums_nonce_frontend');
                $jsData = array(
                'ums_nonce_frontend' => $ums_nonce,
                );
                frameUms::_()->addScript('nonceUms', UMS_JS_PATH . 'nonce.js');
                frameUms::_()->addJSVar('nonceUms', 'UMS_NONCE_FRONTEND', $jsData);
            }
        }
    }
