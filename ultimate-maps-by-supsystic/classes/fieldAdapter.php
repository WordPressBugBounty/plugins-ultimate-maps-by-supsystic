...
class fieldAdapterUms
{
  ...
  public static function taxDataHtml($field)
  {
    ...
    return '<div>' .
      __('Apply To', UMS_LANG_CODE) .
      '
            <div id="tax_address">
                <b>' .
      __('Address', UMS_LANG_CODE) .
      '</b><br />
                ' .
      __('Destination', UMS_LANG_CODE) .
      ':' .
      htmlUms::checkboxlist('params[dest]', ['optionsUms' => $listOfDest]) .
      '<br />
                ' .
      __('Country', UMS_LANG_CODE) .
      ':' .
      htmlUms::countryList('params[country]', ['notSelected' => true, 'value' => $field->value['country']]) .
      '<br />
            </div>
            <div id="tax_category">
                <b>' .
      __('Categories', UMS_LANG_CODE) .
      '</b><br />
                ' .
      (empty($cOptions) ? __('You have no categories', UMS_LANG_CODE) : htmlUms::checkboxlist('params[categories][]', ['optionsUms' => $cOptions])) .
      '<br />
                    <b>' .
      __('Brands', UMS_LANG_CODE) .
      '</b><br />
                ' .
      (empty($bOptions) ? __('You have no brands', UMS_LANG_CODE) : htmlUms::checkboxlist('params[brands][]', ['optionsUms' => $bOptions])) .
      '<br />
            </div>
            <div>' .
      __('Tax Rate', UMS_LANG_CODE) .
      ': ' .
      htmlUms::text('params[rate]', ['value' => $field->value['rate']]) .
      '</div>
            <div>' .
      __('Absolute', UMS_LANG_CODE) .
      ': ' .
      htmlUms::checkbox('params[absolute]', ['checked' => $field->value['absolute']]) .
      '</div>
        </div>';
  }
  ...
  public static function getFieldAttributes($params)
  {
    ...
    if (!empty($params->attr)) {
      foreach ($params->attr as $key => $value) {
        $output .= __($key) . ':<br />';
        $output .= htmlUms::text('params[attr][' . $key . ']', ['value' => $value]) . '<br />';
      }
    } else {
      $output .= __('class', UMS_LANG_CODE) . ':<br />';
      $output .= htmlUms::text('params[attr][class]', ['value' => '']) . '<br />';
      $output .= __('id', UMS_LANG_CODE) . ':<br />';
      $output .= htmlUms::text('params[attr][id]', ['value' => '']) . '<br />';
    }
    return $output;
  }
  /**
  ...
  public static function getCachedStates($clearCache = false)
  {
    if (empty(self::$states) || $clearCache) {
      self::$states = frameUms::_()
        ->getTable('states')
        ->leftJoin(frameUms::_()->getTable('countries'), 'country_id')->getAll('toe_states.id,
					toe_states.name, 
					toe_states.code, 
