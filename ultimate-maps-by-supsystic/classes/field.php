...
class fieldUms
{
   ...
   */
  public $adapt = ['htmlUms' => '', 'dbFrom' => '', 'dbTo' => ''];
  /**
    ...
    if (method_exists('htmlUms', $this->html)) {
      ...
      if ($this->name == 'default_value') {
        ...
        if (!empty($optionsFromDb)) {
          $options = [0 => __('Select', UMS_LANG_CODE)];
  ...
  public function viewField($name, $fieldValue = '')
  {
    ...
    $params = ['optionsUms' => $options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked];
    $output = '';
    if (method_exists('htmlUms', $method)) {
      $output .= htmlUms::$method($name, $params);
      $output .= htmlUms::hidden('extra_field[' . $this->name . ']', ['value' => $this->id]);
    }
    return $output;
  }
  ...
  public function processParams($tag, $id)
  {
    ...
    if ($this->name == 'params') {
      ...
      if ($tag > 0 || $id == 0) {
        ...
        $output .= htmlUms::text('params[width]', ['value' => $width]) . '<br />';
        $output .= __('height', UMS_LANG_CODE) . ':<br />';
        $output .= htmlUms::text('params[height]', ['value' => $height]) . '<br />';
        $output .= '</div>';
      }
  ...
  public function prepareConfigOptions($xml)
  {
    ...
    foreach ($config->params->param as $param) {
      ...
      $config_params[$name] = ['type' => $type, 'label' => $label, 'helperUms' => $helper, 'moduleUms' => $module, 'values' => $values, 'default' => $default, 'description' => $description];
    }
    ...
    if (!empty($configOptions)) {
      ...
      foreach ($params as $param) {
        ...
        foreach ($configOptions as $key => $value) {
          ...
          $htmlParams = ['value' => $fieldValue, 'optionsUms' => $options];
          if ($method == 'checkbox') {
            $htmlParams['value'] = 1;
