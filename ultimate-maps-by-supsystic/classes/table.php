...
abstract class tableUms
{
      ...
      if ($id) {
        ...
        foreach ($data as $field) {
          ...
          foreach ($field as $k => $v) {
            if (isset($this->_fields[$k])) {
              $row[$k] = toeCreateObjUms('fieldUms', [$this->_fields[$k]->name, $this->_fields[$k]->html, $this->_fields[$k]->type, $this->_fields[$k]->default, $this->_fields[$k]->label, $this->_fields[$k]->maxlen, $this->_fields[$k]->description]);
  ...
  {
    $this->_fields[$name] = toeCreateObjUms('fieldUms', [$name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description]);
    return $this;
  }
  /**
  ...
  public function exists($value, $field = '')
  {
    ...
    global $wpdb;
    $res = $wpdb->get_var($wpdb->prepare("SELECT * FROM {$wpdb->prefix}ums_modules WHERE %1s = %s", $field, $value));
    return $res;
  }
