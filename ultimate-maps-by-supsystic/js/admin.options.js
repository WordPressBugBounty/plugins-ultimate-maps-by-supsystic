...
jQuery(window).on('load', function () {
  setTimeout(function () {
    // setTimeout to make sure that all required show/hide were triggered
...
function changeAdminFormUms(formId) {
  if (jQuery.inArray(formId, umsAdminFormChanged) == -1) umsAdminFormChanged.push(formId);
}
...
function umsInitStickyItem() {
  jQuery(window).scroll(function () {
    ...
    for (var i = 0; i < stickiItemsSelectors.length; i++) {
      jQuery(stickiItemsSelectors[i]).each(function () {
        ...
        if (element && element.length && !element.hasClass('sticky-ignore')) {
          ...
          if (element.is(':visible') && currentScrollTop > scrollMinPos && !element.hasClass('supsystic-sticky-active')) {
            // Start sticking
    ...
    if (wasSticking) {
      if (jQuery('#umsPopupGoToTop').length) jQuery('#umsPopupGoToTop').show();
    } else if (wasUnSticking) {
      if (jQuery('#umsPopupGoToTop').length) jQuery('#umsPopupGoToTop').hide();
    }
  });
}
