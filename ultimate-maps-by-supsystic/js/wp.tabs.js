    ...
    activate: function (selector) {
      return this.each(function () {
        ...
        if ($this.find(selector).length) {
          var navigations = $this.find('.nav-tab-wrapper:first').find('a.nav-tab:not(.notTab)');
          if (!this._firstInit) {
            if (this._options.uniqId) $this.find('.umsTabContent').attr('data-tabs-for', this._options.uniqId);
            this._firstInit = 1;
          }
          var allTabsContent = this._options.uniqId ? $this.find('.umsTabContent[data-tabs-for="' + this._options.uniqId + '"]') : $this.find('.umsTabContent');
          allTabsContent.hide();
